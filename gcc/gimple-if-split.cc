...
process_complex_cond (basic_block cond_bb, basic_block then_bb,
		      basic_block else_bb)
{
  gimple *cond = last_nondebug_stmt (cond_bb);
  ...
  if (dump_file && (dump_flags & TDF_DETAILS))
    {
      fprintf (dump_file,
	       "Recognized necessary complex condition in bb %d: ", cond_bb->index);
  ...
  if (dump_file && (dump_flags & TDF_DETAILS))
    {
      fprintf (dump_file, "Successfully transformed:\n (o_cond in bb %d) ",
	       cond_bb->index);
      print_gimple_stmt (dump_file, cond, 0, TDF_NONE);
      fprintf (dump_file, " (i_cond in bb %d) ", inner_cond_bb->index);
			 ...
			 basic_block then_bb)
{
  ...
  int then_bb_succ_edge_flags = 0;
  if (single_succ (then_bb) == EXIT_BLOCK_PTR_FOR_FN (cfun))
    {
      gcc_assert (gimple_code (last_nondebug_stmt (then_bb)) == GIMPLE_RETURN);
      ret_val = gimple_return_retval (as_a<greturn *> (last_nondebug_stmt (then_bb)));
  ...
  if (dump_file && (dump_flags & TDF_DETAILS))
    {
      ...
      print_gimple_stmt (dump_file, last_nondebug_stmt (outer_cond_bb), 0, TDF_NONE);
      fprintf (dump_file, " (i_cond) ");
      print_gimple_stmt (dump_file, last_nondebug_stmt (inner_cond_bb), 0, TDF_NONE);
    }
...
execute_function_over_conditional_bbs (F func)
{
  ...
  for (int i = n_basic_blocks_for_fn (cfun) - NUM_FIXED_BLOCKS - 1; i >= 0; i--)
    {
      gimple *stmt = last_nondebug_stmt (bbs[i]);
...
process_if_split_cfun ()
{
  ...
  execute_function_over_conditional_bbs (
      [] (basic_block cond_bb, basic_block then_bb, basic_block)
      {
...
unsigned int
pass_if_split::execute (function *)
{
