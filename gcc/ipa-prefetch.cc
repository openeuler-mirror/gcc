...
{
  const memref_t *mr1 = *(const memref_t *const *) p1;
  const memref_t *mr2 = *(const memref_t *const *) p2;
...
remap_name (tree name, gimple *stmt, bool is_lhs)
{
  ...
  if (TREE_CODE (name) == SSA_NAME)
	      ...
	      && !DECL_NAME (var)))
	{
	  ...
	  /* So can range-info.  gcc-14 copies range info by source SSA name
	     (vrange_storage), replacing the gcc-12 (kind, range_info) form;
	     see the analogous remap in tree-inline.cc.  */
