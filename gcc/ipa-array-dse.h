...
/* The range this pass works in: a closed interval [MIN, MAX] over trees, or
   the complement of one.  Endpoints need not be constants - the length
   parameter is tracked symbolically as [len, len] - and the RANGE_NINF and
   RANGE_INF sentinels, which are the extremes of RANGE_TYPE, stand for the
   unbounded ends.  A null MIN or MAX means undefined.

   This replaces the value_range the pass was written against.  GCC 13
   dropped both symbolic endpoints and anti-ranges from value_range, and
   this pass needs both: it stores the length parameter symbolically, and
   it inverts a condition's range to describe a false edge.  Everything the
   pass does with a range - the tree arithmetic in ipa-array-dse.cc, the
   comparisons in compare_tree - already works on trees, so the container
   is all that was ever wanted from value_range.  */
...
struct tree_range
{
  ...
  tree_range () : min (NULL_TREE), max (NULL_TREE), anti (false) {}
  tree_range (tree lo, tree hi) : min (lo), max (hi), anti (false) {}

  static tree_range varying (tree type)
  {
    return tree_range (TYPE_MIN_VALUE (type), TYPE_MAX_VALUE (type));
  }

  bool undefined_p () const { return !min || !max; }

  bool cst_p () const
  {
    return min && max && TREE_CODE (min) == INTEGER_CST
  ...
  {
    return !anti && cst_p ()
	   && operand_equal_p (min, TYPE_MIN_VALUE (TREE_TYPE (min)), 0)
	   && operand_equal_p (max, TYPE_MAX_VALUE (TREE_TYPE (max)), 0);
  }

  bool singleton_p () const
  {
    return !anti && min && max && operand_equal_p (min, max, 0);
  }

  void set_varying (tree type) { *this = varying (type); }

  /* Narrow to the intersection with O.  Only constant endpoints narrow; on
     symbolic ones the range is left alone, which is what the container this
     replaces did whenever it could not represent the result.  An anti-range
     narrows only where its hole meets an endpoint, for the same reason.  */
  void intersect (const tree_range &o)
  {
    if (o.undefined_p ())
      return;
    if (undefined_p ())
	...
	*this = o;
    ...
    if (anti || !cst_p () || !o.cst_p ())
      return;
    if (o.anti)
      {
	tree type = TREE_TYPE (min);
	tree one = build_int_cst (type, 1);
	if (tree_int_cst_equal (o.min, o.max))
	  {
	    if (tree_int_cst_equal (o.min, min))
	      min = int_const_binop (PLUS_EXPR, min, one);
	    else if (tree_int_cst_equal (o.min, max))
	      max = int_const_binop (MINUS_EXPR, max, one);
	  }
	return;
      }
    if (tree_int_cst_lt (min, o.min))
      min = o.min;
    if (tree_int_cst_lt (o.max, max))
      max = o.max;
  }

  /* Complement, as the false edge of a condition needs.  A half-open
     interval inverts to the other half; anything else becomes an
     anti-range, or undefined when there is nothing left to say.  */
  void invert ()
  {
    if (undefined_p ())
      return;
    ...
    tree type = TREE_TYPE (min);
    tree one = build_int_cst (type, 1);
    bool lo_open = operand_equal_p (min, TYPE_MIN_VALUE (type), 0);
    bool hi_open = operand_equal_p (max, TYPE_MAX_VALUE (type), 0);
    if (lo_open && hi_open)
      min = max = NULL_TREE;
    else if (lo_open)
      {
...
private:
  ...
  tree_range calc_tree_range (basic_block bb, tree t);
  bool get_factor (tree t, double &factor);
  bool positive_factor_p (tree t);
  tree_range build_range (basic_block bb, tree_code op,
  ...
  void calc_len_range ();
  void update_len_range (basic_block start, const tree_range &new_range);
  tree_range invert_range (const tree_range &range) const;
  tree_range get_range_from_cond (gimple *stmt);
  tree_range get_var_range (basic_block bb, tree offset);
  bool calc_ptr_range ();
...
private:
  ...
  hash_map<basic_block, tree_range> len_range_map;
  std::map<tree, std::map<basic_block, tree_range>> var_range;
...
private:
  ...
  tree_range calc_ref_range (tree var);
  tree_range calc_addr_range (tree var);
  tree_range calc_offset_range (tree var);
  tree_range calc_simple_loop_range (tree var);
