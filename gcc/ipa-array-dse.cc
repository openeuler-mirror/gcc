...
static inline tree
range_min (const tree_range &r)
{
  return r.min;
}

static inline tree
range_max (const tree_range &r)
{
  return r.max;
}

static inline tree_range
range_of (tree min, tree max)
{
  return tree_range (min, max);
}
...
{
  return range_of (build_value (min), build_value (max));
}
...
array_dse_callee::mult_tree (basic_block bb, tree t1, tree t2)
{
  ...
  if (tree_to_shwi (range_min (range1)) < 0)
    return nullptr;
...
{
  if (!t)
    return tree_range::varying (RANGE_TYPE);
  ...
  gcc_assert (len > 0);
  tree_range range1 = calc_tree_range (bb, TREE_OPERAND (t, 0));
  tree_range range2;
  if (len == 2)
  ...
  switch (TREE_CODE (t))
      ...
      case MINUS_EXPR:
	{
	  ...
	  auto len_min = tree_to_shwi (range_min (len_range));
	  auto len_max = tree_to_shwi (range_max (len_range));
...
array_dse_callee::build_range (basic_block bb, tree_code op,
			       const tree_range &r1, const tree_range &r2)
{
  tree min = nullptr;
  tree max = nullptr;
  switch (op)
    {
      case NEGATE_EXPR:
	min = negate_tree (range_max (r1));
	max = negate_tree (range_min (r1));
	break;
      case PLUS_EXPR:
	[[fallthrough]];
      case POINTER_PLUS_EXPR:
	min = plus_tree (range_min (r1), range_min (r2));
	max = plus_tree (range_max (r1), range_max (r2));
	break;
      case MINUS_EXPR:
	[[fallthrough]];
      case POINTER_DIFF_EXPR:
	min = minus_tree (range_min (r1), range_max (r2));
	max = minus_tree (range_max (r1), range_min (r2));
	break;
      case MULT_EXPR:
	min = mult_tree (bb, range_min (r1), range_min (r2));
	max = mult_tree (bb, range_max (r1), range_max (r2));
	break;
      case TRUNC_DIV_EXPR:
	min = div_tree (bb, range_min (r1), range_max (r2));
	max = div_tree (bb, range_max (r1), range_min (r2));
	break;
      case LSHIFT_EXPR:
	min = lshift_tree (range_min (r1), range_min (r2));
	max = lshift_tree (range_max (r1), range_max (r2));
	break;
      case RSHIFT_EXPR:
	min = rshift_tree (range_min (r1), range_max (r2));
	max = rshift_tree (range_max (r1), range_min (r2));
	break;
      case MAX_EXPR:
	min = max_tree (bb, range_min (r1), range_min (r2));
	max = max_tree (bb, range_max (r1), range_max (r2));
	break;
      case MIN_EXPR:
	min = min_tree (bb, range_min (r1), range_min (r2));
	max = min_tree (bb, range_max (r1), range_max (r2));
	break;
      default:
	break;
    }

  return min && max ? range_of (min, max) : tree_range::varying (RANGE_TYPE);
}
...
array_dse_callee::compare_tree_by_minus (basic_block bb, tree t1, tree t2)
{
  ...
  HOST_WIDE_INT min = tree_to_shwi (range_min (range));
  HOST_WIDE_INT max = tree_to_shwi (range_max (range));
  if (min == 0)
    return GE;
  if (min > 0)
    return GT;
...
tree_range
array_dse_callee::invert_range (const tree_range &range) const
{
...
array_dse_callee::get_var_range (basic_block bb, tree var)
{
  ...
  if (var == len_main_var)
    return range_of (signed_len_var, signed_len_var);
  ...
  if (gimple_code (stmt) == GIMPLE_PHI)
      ...
      for (unsigned i = 1; i < gimple_phi_num_args (stmt); i++)
	{
	  ...
	  tree min = min_tree (bb, range_min (range), range_min (arg_range));
	  tree max = max_tree (bb, range_max (range), range_max (arg_range));
	  if (!min || !max)
	    {
	      range.set_varying (RANGE_TYPE);
	      break;
  ...
  tree_range range1 = get_var_range (bb, rhs1);
  tree_range range2 = tree_range::varying (RANGE_TYPE);
  if (rhs2)
      ...
      for (auto gsi = gsi_start_bb (bb); !gsi_end_p (gsi); gsi_next (&gsi))
	{
	  ...
	  for (unsigned i = 0; i < gimple_num_ops (stmt); i++)
		...
		continue;
	      ...
	      /* offset >= 0.  */
	      auto ret = compare_tree (bb, range_min (range), integer_zero_node);
	      if (!ret || ret & LT)
		return false;
	      ...
	      ret = compare_tree (bb, range_max (range), tmp);
	      if (!ret || ret & GT)
		return false;
	    }
	}
  ...
  for (unsigned i = 0; i < array_args.length (); i++)
    {
      ...
      /* Check requirement 2.  */
      auto ret = compare_tree (bb, range_min (len_range), integer_one_node);
      if (!ret || ret & ~GE)
	return false;
      ...
      if (is_tail_recursive_call[i])
	{
	  ...
	  ret = compare_tree (bb, range_min (array_range), integer_zero_node);
	  if (!ret || ret & ~GE)
	    return false;
	}
      else
	{
	  ...
	  if (!integer_zerop (range_min (array_range))
	      || !integer_zerop (range_max (array_range)))
	    return false;
	}
...
array_dse_callee::update_branch_range (basic_block bb)
{
  ...
  tree min = range_min (range);
  tree max = range_max (range);
  HOST_WIDE_INT value = TREE_INT_CST_LOW (rhs);
  ...
  var_range[lhs][bb] = range_of (min, max);
}
  ...
  if (is_gimple_assign (stmt)
      ...
      if (TREE_CODE (rhs2) == SSA_NAME)
	{
	  ...
	  if (is_gimple_assign (stmt)
	    ...
	    recursive_ptr_max = range_max (get_var_range (bb, rhs1));
	}
    }
  ...
  if (!recursive_ptr_max)
    {
      ...
      recursive_ptr_max = plus_tree (range_max (range1), range_max (range2));
    }
...
array_dse_callee::calc_loop_var_range (loop_p loop)
{
  ...
  for (auto gsi = gsi_start_phis (header); !gsi_end_p (gsi); gsi_next (&gsi))
	...
	return false;
      ...
      tree min = step > 0 ? range_min (init_range) : build_value (RANGE_NINF);
      tree max = step > 0 ? build_value (RANGE_INF) : range_max (init_range);
      auto new_range = range_of (min, max);
      for (auto bb : get_all_dominated_blocks (CDI_DOMINATORS, header))
	{
	  ...
	  min = max_tree (bb, min, range_min (range));
	  max = min_tree (bb, max, range_max (range));
	  if (!min || !max)
	    return false;
	  range = range_of (min, max);
	}
  ...
  for (auto bb : get_all_dominated_blocks (CDI_DOMINATORS, loop->header))
    {
      ...
      tree lhs_min = range_min (lhs_range);
      tree lhs_max = range_max (lhs_range);
      tree rhs_min = range_min (rhs_range);
      tree rhs_max = range_max (rhs_range);
	...
	var_range[lhs][bb] = range_of (lhs_min, lhs_max);
      if (loop_var_p (loop, rhs) || iterate_var_p (loop, rhs))
	var_range[rhs][bb] = range_of (rhs_min, rhs_max);
...
array_dse_edge::calc_read_bound ()
{
  for (auto [var, stmt] : array_accesses)
	...
	continue;
	  ...
	  || !integer_cst_p (range_max (range)))
	return false;

      auto max = tree_to_shwi (range_max (range));
      if (max % elem_size)
	return false;
...
array_dse_edge::calc_ref_range (tree var)
{
  ...
  if (code == ARRAY_REF)
	...
	return r;
      ...
      gcc_assert (integer_cst_p (range_min (r)) && integer_cst_p (range_max (r)));
      return make_range (tree_to_shwi (range_min (r)) * elem_size,
			 tree_to_shwi (range_max (r)) * elem_size);
    }
  ...
  return range_of (plus_tree (range_min (r1), range_min (r2)), plus_tree (range_max (r1), range_max (r2)));
}
...
array_dse_edge::calc_offset_range (tree offset)
{
  ...
  if (!integer_cst_p (range_min (range)) || !integer_cst_p (range_max (range)))
    return tree_range::varying (RANGE_TYPE);

  return make_range (tree_to_shwi (range_min (range)) * elem_size,
		     tree_to_shwi (range_max (range)) * elem_size);
}
...
array_dse_edge::calc_simple_loop_range (tree var)
{
  ...
  tree_range init_range;
  if (array_addr_p (init_var))
    init_range = calc_addr_range (init_var);
  else
    init_range = calc_offset_range (init_var);

  if (!init_range.singleton_p () || !integer_cst_p (range_min (init_range)))
    return tree_range::varying (RANGE_TYPE);

  HOST_WIDE_INT init_value = tree_to_shwi (range_min (init_range));
...
array_dse_edge::calc_array_arg_start ()
{
  ...
  if (!range.singleton_p () || !integer_cst_p (range_min (range)))
    return false;

  auto value = tree_to_shwi (range_min (range));
  if (value % elem_size)
    return false;
...
array_dse_edge::check_optimized_area_rewrite ()
{
  ...
  len_arg_min = tree_to_shwi (range_min (range));

  return tree_to_shwi (range_max (range)) + array_arg_start <= read_upper_bound;
}
