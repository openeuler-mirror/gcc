...
/* The two counts below assert the dynamic memory access checks: the pass
   guards the speculative dereferences it inserts, so the address it
   evaluates ahead of time is only read when it lands on the page the
   original access already touched.  */
