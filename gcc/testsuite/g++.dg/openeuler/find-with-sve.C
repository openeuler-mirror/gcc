...
/* The test source carried from gcc-12 uses a C++17 if-with-initializer in
   test_set() while forcing -std=c++11, so the compiler emits a
   -Wc++17-extensions warning; gcc-12.3.1 emits the identical warning, so
   the test failed "excess errors" in the packaged set there too.  Prune
   the noise instead of touching the test body.  */
/* { dg-prune-output "init-statement in selection statements" } */
