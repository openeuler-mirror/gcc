...
check_cond_stmt (gimple *stmt)
{
  ...
  /* Only NE_EXPR: the rebuilt loops reuse the condition code verbatim with
     hard-wired edge polarity, so an EQ_EXPR compare (an else-break form,
     after gimplification) would be rewritten with inverted semantics.  */
...
record_origin_loop_exit_info (class loop *loop)
{
  ...
  /* The rebuilt loops hard-wire the exit orientation: the length compare
     leaves the loop on its false edge, the byte compare on its true edge.
     A candidate with the opposite orientation (e.g. NE_EXPR breaking on
     the false edge) would be rewritten inverted and feed an all-zero XOR
     to __builtin_ctzll, so reject it.  */
  if (found
      && ((origin_loop.exit_e1->flags & EDGE_FALSE_VALUE) == 0
	  || (origin_loop.exit_e2->flags & EDGE_TRUE_VALUE) == 0))
    found = false;
...
static bool
record_origin_loop_body (class loop *)
{
...
check_exit_bb (class loop *loop)
{
  ...
  for (gsi = gsi_start_phis (origin_loop.exit_bb1); !gsi_end_p (gsi);
	...
	continue;
      /* rewrite_add_phi_arg can only fill exit-block PHI arguments for
	 the base induction variable; any other real PHI (a flag set on
	 break, another loop's induction variable) would be left with
	 empty argument slots on the new edges - malformed gimple.  */
...
/* Set when determine_loop_form split a back edge to canonicalize a
   candidate; the pass then requests a CFG cleanup so that a forwarder
   block left on a rejected candidate is removed.  */
static bool latch_split_performed;
...
{
  /* The analysis below expects the canonical header/body/latch form with an
     empty latch.  Early CFG cleanup may have merged the empty latch into the
     body block, leaving a 2-bb loop whose latch carries the byte compare and
     an exit edge; restore the canonical form by splitting the back edge.
     Loop exits are not recorded at this point, so count them explicitly
     rather than via single_exit.  */
  if (!loop->inner && loop->num_nodes == 2
      && loop->latch && !empty_block_p (loop->latch)
      && !(loop_latch_edge (loop)->flags & (EDGE_ABNORMAL | EDGE_EH))
      && get_loop_exit_edges (loop).length () == 2)
    {
      split_edge (loop_latch_edge (loop));
      latch_split_performed = true;
    }
			   ...
			   class loop *outer)
{
  ...
  /* The 8-byte loads replace byte accesses at arbitrary offsets: give them
     an alias-set-0 access path (ref-all, matching the chars they replace)
     and a 1-byte-aligned access type, so TBAA cannot disambiguate them
     against the surrounding code and expansion never assumes a naturally
     aligned word.  */
  tree wide_type = build_aligned_type (long_unsigned_type_node,
				       BITS_PER_UNIT);
  tree wide_ptr_type = build_pointer_type_for_mode (char_type_node,
						    ptr_mode, true);
...
tree_ssa_array_widen_compare ()
{
  ...
  /* A back edge split on a candidate that was then rejected leaves an
     empty forwarder block behind; ask for the cleanup ourselves instead
     of relying on a later pass's TODO to remove it.  */
  if (latch_split_performed)
...
{
  /* The rewritten loop locates the first mismatching byte by counting
     trailing zeros in the XOR of two 8-byte loads, which maps bit position
     to byte position correctly only on little-endian targets.  */
